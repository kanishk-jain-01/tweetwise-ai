 ...
 */
export function compressImageIfNeeded(base64Data: string, maxSizeBytes: number = 4 * 1024 * 1024): string {
  ...
  console.log(`Image too large (${Math.round(originalBuffer.length / 1024)}KB), compressing with ratio ${compressionRatio.toFixed(2)}`);
...
export class TwitterMediaUploader {
  ...
  private async throttleRequest(): Promise<void> {
    ...
    if (timeSinceLastRequest < this.minRequestInterval) {
      ...
      console.log(`Throttling request, waiting ${waitTime}ms to avoid rate limiting`);
   ...
   */
  async uploadImage(base64Data: string, altText?: string): Promise<MediaUploadResult> {
    try {
      ...
      console.log(`Uploading image: ${Math.round(imageBuffer.length / 1024)}KB, MIME: ${mimeType}`);
      ...
      if (imageBuffer.length > maxSizeBytes) {
        throw new Error(`Image size (${Math.round(imageBuffer.length / 1024)}KB) is too large. Please try a smaller image.`);
      }
      ...
      formData.append('media', imageBlob, `image.${getFileExtensionFromMimeType(mimeType)}`);
      ...
      if (!response.ok) {
        ...
        console.error('Twitter v2 media upload error:', response.status, errorData);
        
        // Handle specific HTTP status codes
        if (response.status === 400) {
          console.error('400 Bad Request error details:', errorData);
          throw new Error(`Invalid request: ${errorData.detail || errorData.errors?.[0]?.message || 'Please check image format and size'}`);
        }
        ...
        if (response.status === 403) {
          ...
          if (errorData.detail?.includes('scope') || errorData.title?.includes('scope') || 
              errorData.errors?.[0]?.message?.includes('scope')) {
            throw new Error('Missing media.write permission. Please disconnect and reconnect your Twitter account to get the required permissions.');
          }
          
          // Check if it's specifically about media upload permissions
          if (errorData.title?.includes('media') || errorData.detail?.includes('media')) {
            throw new Error('Media upload not permitted. Please disconnect and reconnect your Twitter account with media upload permissions.');
          }
          
          throw new Error('Access denied. Your Twitter account may need additional permissions. Please disconnect and reconnect your account.');
        }
        if (response.status === 413) {
          throw new Error('Image is too large. Twitter supports images up to 5MB.');
        }
        if (response.status === 415) {
          throw new Error('Unsupported image format. Please use JPEG, PNG, GIF, or WebP.');
        }
        if (response.status === 429) {
          throw new Error('Twitter media upload rate limit exceeded. Please try again later.');
        }
        
        throw new Error(`Twitter media upload failed: ${response.status} ${response.statusText}`);
      }

      const uploadResult = await response.json();
      
      // Debug: Log the actual response from Twitter
      console.log('Twitter media upload response:', JSON.stringify(uploadResult, null, 2));
      
      // Extract media ID from Twitter v2 API response structure
      const mediaId = uploadResult.data?.id || uploadResult.media_id || uploadResult.media_id_string;
      const mediaKey = uploadResult.data?.media_key;
      
      // Validate response structure
      if (!mediaId) {
        console.error('Missing media ID in response. Full response:', uploadResult);
        throw new Error(`Invalid response from Twitter media upload API. No media ID found. Response: ${JSON.stringify(uploadResult)}`);
      }

      console.log(`Media upload successful! Media ID: ${mediaId}, Media Key: ${mediaKey}`);
      ...
      throw new Error(`Failed to upload image to Twitter: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async checkMediaStatus(mediaId: string): Promise<any> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to check media status: ${response.status} ${response.statusText}`);
      }
